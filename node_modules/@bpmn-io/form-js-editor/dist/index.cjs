...
function FormEditor$1(props) {
  ...
  const onReset = hooks$1.useCallback(() => {}, []); // attach default palette
...
Keyboard.prototype.bind = function (node) {
  ...
  minDom.event.bind(node, 'keydown', this._keydownHandler, true);
  minDom.event.bind(node, 'keyup', this._keyupHandler, true);
...
Keyboard.prototype.unbind = function () {
  ...
  if (node) {
    ...
    minDom.event.unbind(node, 'keydown', this._keydownHandler, true);
    minDom.event.unbind(node, 'keyup', this._keyupHandler, true);
  }
...
class AddFormFieldHandler {
  ...
  execute(context) {
    ...
    arrayAdd$1(minDash.get(schema, targetPath), targetIndex, formField); // (2) Update paths of new form field and its siblings

    minDash.get(schema, targetPath).forEach((formField, index) => updatePath(this._formFieldRegistry, formField, index)); // (3) Add new form field to form field registry
  ...
  revert(context) {
    ...
    arrayRemove(minDash.get(schema, targetPath), targetIndex); // (2) Update paths of new form field and its siblings

    minDash.get(schema, targetPath).forEach((formField, index) => updatePath(this._formFieldRegistry, formField, index)); // (3) Remove new form field from form field registry
...
class MoveFormFieldHandler {
  ...
  moveFormField(context, revert) {
    ...
    if (sourceFormField.id === targetFormField.id) {
      ...
      arrayMove.mutate(minDash.get(schema, sourcePath), sourceIndex, targetIndex); // (2) Update paths of new form field and its siblings
      ...
      arrayRemove(minDash.get(schema, sourcePath), sourceIndex); // (2) Update paths of siblings

      minDash.get(schema, sourcePath).forEach((formField, index) => updatePath(this._formFieldRegistry, formField, index));
      const targetPath = [...targetFormField._path, 'components']; // (3) Add form field

      arrayAdd$1(minDash.get(schema, targetPath), targetIndex, formField); // (4) Update paths of siblings
...
class RemoveFormFieldHandler {
  ...
  execute(context) {
    ...
    const sourcePath = [...sourceFormField._path, 'components'];
    const formField = context.formField = minDash.get(schema, [...sourcePath, sourceIndex]); // (1) Remove form field

    arrayRemove(minDash.get(schema, sourcePath), sourceIndex); // (2) Update paths of its siblings

    minDash.get(schema, sourcePath).forEach((formField, index) => updatePath(this._formFieldRegistry, formField, index)); // (3) Remove form field from form field registry
  ...
  revert(context) {
    ...
    arrayAdd$1(minDash.get(schema, sourcePath), sourceIndex, formField); // (2) Update paths of its siblings

    minDash.get(schema, sourcePath).forEach((formField, index) => updatePath(this._formFieldRegistry, formField, index)); // (3) Add form field to form field registry
